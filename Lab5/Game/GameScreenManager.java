...
public class GameScreenManager {
    public void displayBoard(Board board, Player player1, Player player2, Player currentPlayer, int currentBoardIndex, List<Board> boards, int[] roundCounts) {
        if (board == null) {
        ...
        for (int row = 0; row < Board.SIZE; row++) {
            ...
            if (row == 2 && board.isGomokuMode()) {
                if (roundCounts.length > currentBoardIndex) {
                String roundDisplay = "Current Round: " + roundCounts[currentBoardIndex];
                System.out.printf("    %s", roundDisplay);
                }
            }

            // 打印分隔空格
            if (row == 0 || row == 1 || (row == 2 && board.isGomokuMode())) {
            ...
            // 在特定行打印 Board List
            if (row < boards.size()) {
                String mode = getBoardMode(boards.get(row));
                // 使用 printf 实现右对齐
        ...
        if (board.isReversiMode() && currentPlayer != null &&!board.hasValidMove(currentPlayer)) {
    ...
    private String getBoardMode(Board board) {
        if (board.isReversiMode()) {
            return "reversi";
        } else if (board.isGomokuMode()) {
            return "gomoku";
